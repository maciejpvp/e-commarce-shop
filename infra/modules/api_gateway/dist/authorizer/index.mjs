...
var nodeWebCompat = {
  ...
  verifySignatureSync: ({ alg, keyObject, jwsSigningInput, signature }) => alg !== "EdDSA" ? (
    ...
    createVerify(JwtSignatureAlgorithmHashNames[alg]).update(jwsSigningInput).verify({
      key: keyObject,
...
var CognitoJwtVerifier = class _CognitoJwtVerifier extends JwtVerifierBase {
   ...
   */
  static parseUserPoolId(userPoolId2) {
    const match = userPoolId2.match(this.USER_POOL_ID_REGEX);
    if (!match) {
      throw new ParameterValidationError(`Invalid Cognito User Pool ID: ${userPoolId2}`);
    }
    const region = match.groups.region;
    const issuer = `https://cognito-idp.${region}.amazonaws.com/${userPoolId2}`;
    return {
      issuer,
  ...
  cacheJwks(...[jwks, userPoolId2]) {
    let issuer;
    if (userPoolId2 !== void 0) {
      issuer = _CognitoJwtVerifier.parseUserPoolId(userPoolId2).issuer;
...
// src/authorizers/group_authorizer.ts
...
var handler = async (event) => {
  ...
  try {
    ...
    const resourceMap = JSON.parse(process.env.RESOURCE_GROUP_MAPPING || "{}");
    ...
    return generatePolicy(payload.sub, isAuthorized ? "Allow" : "Deny", methodArn);
  } catch (err) {
